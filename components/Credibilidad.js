...
const partners = [
  {
    ...
    logo: (
      <svg viewBox="0 0 272 92" className="h-6 w-auto" fill={GREY} xmlns="http://www.w3.org/2000/svg">
  ...
  },
  {
    ...
    logo: (
      <div className="flex items-center gap-2" style={{ color: GREY }}>
        <svg viewBox="0 0 64 64" className="h-6 w-6 shrink-0" fill="currentColor">
          <rect x="1" y="1" width="30" height="30"/>
          <rect x="33" y="1" width="30" height="30"/>
          <rect x="1" y="33" width="30" height="30"/>
          <rect x="33" y="33" width="30" height="30"/>
        </svg>
        <span className="text-xl font-semibold leading-none">Microsoft</span>
      </div>
    ),
  },
  {
  ...
  {
    name: 'Red.es',
    description: 'Alineados con la estrategia nacional de capacitación e inclusión digital.',
    logo: (
      <span className="text-3xl font-extrabold tracking-tight leading-none" style={{ color: GREY }}>
        red.es
      </span>
    ),
  },
  {
    name: 'Fundación Pro Bono España',
    description: 'Recibimos asesoramiento gratuito en aspectos legales, administrativos y de gestión institucional.',
    logo: (
      <div className="flex items-center gap-2" style={{ color: GREY }}>
        <svg viewBox="0 0 44 44" className="h-7 w-7 shrink-0" fill="currentColor">
          <rect x="2" y="14" width="10" height="10"/>
          <rect x="12" y="24" width="10" height="10"/>
          <rect x="22" y="14" width="10" height="10"/>
          <rect x="12" y="4" width="10" height="10"/>
          <rect x="12" y="34" width="10" height="10" fillOpacity="0.8"/>
          <rect x="32" y="24" width="10" height="10"/>
        </svg>
        <span className="text-base font-semibold leading-none whitespace-nowrap">Pro Bono España</span>
      </div>
    ),
  },
]
    ...
    <section className="relative pt-8 pb-16 md:pt-10 md:pb-20 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div ref={ref} className={`reveal ${visible ? 'visible' : ''}`}>
          ...
          <div className={`stagger-children grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-5 ${visible ? 'visible' : ''}`}>
            {partners.map((p, i) => (
              <div
                key={i}
                className="flex flex-col items-center text-center p-6 rounded-2xl border border-gray-100 bg-gray-50/60 hover:bg-white hover:border-gray-200 transition-colors duration-300"
              >
                <div className="h-10 w-full flex items-center justify-center mb-5 overflow-visible">
                  {p.logo}
                </div>
